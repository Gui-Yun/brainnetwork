...
def rr_selection(trials, labels, t_stimulus=t_stimulus, l_stimulus=l_stimulus, 
    ...
    for target_class in target_classes:
        ...
        # 5. 加入到总集合中 (Union 操作)
        union_rr_neurons.update(class_rr_indices)
        
        print(f"  - 类别 {target_class} 贡献了 {len(class_rr_indices)} 个 RR 神经元")

    # 将集合转回列表
    rr_neurons = list(union_rr_neurons)
    
    print(f"三类并集后共筛选出 {len(rr_neurons)} 个唯一 RR 神经元")

    # 6. 限制最大数量 (Random Shuffle & Clip)
    if len(rr_neurons) > max_n:
        np.random.seed(101)
        np.random.shuffle(rr_neurons)
        rr_neurons = rr_neurons[:max_n]
        print(f"  - 随机下采样至 {max_n} 个")

    
    return rr_neurons
def rr_selection_old(trials, labels, t_stimulus=t_stimulus, l=l_stimulus, alpha_fdr=0.05, alpha_level=0.05, reliability_threshold=reliability_threshold, snr_threshold=snr_threshold, effect_size_threshold=0.5, response_ratio_threshold=0.6, max_n = 2000):
    """
    快速RR神经元筛选
    优化策略:
    1. 向量化计算替代循环
