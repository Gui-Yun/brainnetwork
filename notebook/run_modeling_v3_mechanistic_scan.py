...
class SearchSpace:
    ...
    lambda_norm: tuple[float, float]
    g_c: tuple[float, float]
...
def participants_ratio_from_two_conditions(
    ...
    rr_sets = rr_selection_class_for_model(
        all_trials,
        labels,
    ...
    rw = slice(int(response_start), int(response_end))
    ...
    for cond_name, cls, c_trials in [("coherent", 1, tc), ("random", 3, tr)]:
        ...
        if len(oth) == 0:
            out[cond_name] = float(np.nanmean(m_rr[rw]))
            continue
        m_oth = np.nanmean(c_trials[:, oth, :], axis=(0, 1))
        den = float(np.nanmean(m_oth[rw]))
        out[cond_name] = np.nan if (not np.isfinite(den) or abs(den) <= EPS) else float(np.nanmean(m_rr[rw]) / den)
    return out
...
def build_network(params: ModelParams, rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    np.fill_diagonal(core_mask, False)
    core_conn = rng.random((n, n)) < float(params.core_prob)
    core_conn = core_conn & core_mask
    W[core_conn] += np.abs(rng.normal(loc=float(params.w_core), scale=0.25 * float(params.w_core), size=int(np.sum(core_conn))))

    peri_mask = np.ones((n, n), dtype=bool)
    np.fill_diagonal(peri_mask, False)
    peri_conn = (rng.random((n, n)) < float(params.peri_prob)) & peri_mask
    W[peri_conn] += rng.normal(
        loc=float(params.w_peri_mean),
        scale=float(params.w_peri_std),
        size=int(np.sum(peri_conn)),
    )

    bal_vec = np.abs(rng.normal(size=n))
    bal_vec = bal_vec / (np.linalg.norm(bal_vec) + EPS)
    W -= float(params.w_bal) * np.outer(bal_vec, bal_vec)

    np.fill_diagonal(W, 0.0)
    W = normalize_spectral_radius(W, target_radius=float(params.target_radius))
    ...
    v = float(params.beta_core) * v_core + float(params.beta_peri) * v_peri
    v = v / (np.linalg.norm(v) + EPS)

    return W, B, u_shared, v, core_idx
...
def simulate_condition(
    ...
    for tr in range(n_trials):
        ...
        s_cond = ar1_series(T, rho=params.cond_rho, scale=1.0, rng=rng)
        eta = ar1_series(T, rho=params.noise_rho, scale=1.0, rng=rng)
        r = np.zeros(n, dtype=float)
        for t in range(T):
            i_stim = B @ z[t]
            if condition == "coherent":
                i_cond = float(params.g_c) * v_cond * s_cond[t]
            else:
            ...
            norm_term = float(params.lambda_norm) * float(params.gamma_norm) * float(np.mean(r))
            inp = W @ r + i_stim + i_cond + xi_ind + xi_shared - norm_term
...
def allocation_metrics_from_trial_matrix(
    ...
    # Keep the previous active-count-based ratio as a fallback, but gini follows
    # historical trial-level definition (mean trial-wise Gini_Mean).
    gini_trials = np.asarray([gini_index(X[i]) for i in range(X.shape[0])], dtype=float)
    out = {
        "participants_ratio": float(active_count / max(1, n_core)),
...
def simulate_one_param_set(params: ModelParams, seed: int) -> dict:
    ...
    W, B, u_shared, v_cond, core_idx = build_network(params, rng=rng)

    trials_rand = simulate_condition(params, W=W, B=B, u_shared=u_shared, v_cond=v_cond, condition="random", rng=rng)
    trials_coh = simulate_condition(params, W=W, B=B, u_shared=u_shared, v_cond=v_cond, condition="coherent", rng=rng)
    ...
    if np.isfinite(pr_rand):
        met_rand["participants_ratio"] = pr_rand
    if np.isfinite(pr_coh):
        met_coh["participants_ratio"] = pr_coh

    delta = {f"{k}_delta": float(met_coh.get(k, np.nan) - met_rand.get(k, np.nan)) for k in met_rand.keys()}
    out = {
...
def sample_param_set(base: ModelParams, rng: np.random.Generator, space: SearchSpace) -> ModelParams:
    return ModelParams(
        ...
        lambda_norm=_sample_range(rng, space.lambda_norm),
        gamma_norm=base.gamma_norm,
        g_c=_sample_range(rng, space.g_c),
...
def model_params_to_row(prefix: str, p: ModelParams) -> dict:
    d = {
        ...
        f"{prefix}lambda_norm": p.lambda_norm,
        f"{prefix}g_c": p.g_c,
...
def run_search(
    ...
    for idx in range(int(args.num_samples)):
        ...
        s_fc, e_fc = score_group(avg_delta, targets["fc"], fc_keys)
        ...
        s_geom, e_geom = score_group(
            avg_delta, targets["geom"], geom_keys, metric_weights=geom_metric_weights
        )
...
def plot_search_overview(df: pd.DataFrame, out_dir: str):
    ...
    ax = axes[1]
    ax.scatter(df["g_c"], df["lambda_norm"], c=df["score_total"], s=18, cmap="viridis", alpha=0.85)
    ax.set_xlabel("g_c")
    ax.set_ylabel("lambda_norm")
    style_axis(ax, grid=True)
    ...
    save_variants(fig, os.path.join(out_dir, "model_v3_search_overview.png"))
...
def plot_best_vs_target(best_row: pd.Series, targets: dict, out_dir: str):
    ...
    style_axis(ax, grid=True)
    save_variants(fig, os.path.join(out_dir, "model_v3_best_vs_target_deltas.png"))


def parse_args():
    p = argparse.ArgumentParser(description="Mechanistic v3 modeling scan (coherent-drive + stabilized recurrent dynamics).")
    p.add_argument("--results-dir", type=str, default=DEFAULT_RESULTS_DIR)
    p.add_argument("--num-samples", type=int, default=480)
    p.add_argument("--repeats-per-param", type=int, default=5)
    ...
    p.add_argument("--w-fc", type=float, default=0.45)
    p.add_argument("--w-alloc", type=float, default=0.40)
    p.add_argument("--w-geom", type=float, default=0.15)
    p.add_argument("--metric-w-participants-ratio", type=float, default=3.0)
    p.add_argument("--metric-w-gini", type=float, default=1.0)
    p.add_argument("--metric-w-orth-parallel-ratio", type=float, default=3.0)
    p.add_argument("--metric-w-var-orthogonal", type=float, default=1.0)
    p.add_argument("--metric-w-var-parallel", type=float, default=1.0)
    p.add_argument("--metric-w-mean-rsm", type=float, default=1.2)

    p.add_argument("--score-cap-fc", type=float, default=8.0)
    p.add_argument("--score-cap-alloc", type=float, default=8.0)
    p.add_argument("--score-cap-geom", type=float, default=2.0)
    ...
    p.add_argument("--penalty-participants-w", type=float, default=1.0)
    p.add_argument("--penalty-rsm-max", type=float, default=0.0)
    p.add_argument("--penalty-rsm-w", type=float, default=0.75)
    ...
    p.add_argument("--min-amp-penalty-w-participants", type=float, default=1.2)
    ...
    p.add_argument("--min-amp-penalty-w-orth-ratio", type=float, default=0.8)
    p.add_argument("--max-amp-ratio-orth-ratio", type=float, default=2.00)
    p.add_argument("--max-amp-penalty-w-orth-ratio", type=float, default=1.8)
    ...
    p.add_argument("--lambda-norm-min", type=float, default=0.05)
    p.add_argument("--lambda-norm-max", type=float, default=4.50)
    p.add_argument("--g-c-min", type=float, default=0.05)
    p.add_argument("--g-c-max", type=float, default=4.50)
...
def build_search_space(args) -> SearchSpace:
    return SearchSpace(
        ...
        lambda_norm=_checked_range("lambda_norm", args.lambda_norm_min, args.lambda_norm_max),
        g_c=_checked_range("g_c", args.g_c_min, args.g_c_max),
...
def main():
    ...
    scan_csv = os.path.join(group_dir, "group_model_v3_scan_summary.csv")
    detail_csv = os.path.join(group_dir, "group_model_v3_scan_repeats_long.csv")
    ...
    top_csv = os.path.join(group_dir, "group_model_v3_top_params.csv")
    ...
    report_path = os.path.join(group_dir, "Group_Modeling_v3_Report.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Group Modeling v3 Report\n\n")
    ...
    print("====== Modeling v3 mechanistic scan completed ======")
